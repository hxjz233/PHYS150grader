...
def generate_manual_feedback(config_manager: ConfigManager = None, footer: str = None):
    """Generate feedback files from test results CSV files.
    
    Args:
        config_manager: Optional ConfigManager instance
        footer: Optional custom footer message to replace default footer
    """
    ...
    summary_scores = {}  # {prob_idx: [scores]}
    problem_failures = {}  # {prob_idx: failure_count}
    detailed_failures = []  # List of (userid, prob_idx, fail_msg)
            ...
            for prob_idx, problem in enumerate(config.tester["problem"], 1):
                ...
                for t in range(ntests):
                    ...
                    if pf == '1':
                        ...
                        problem_failures[prob_idx] = problem_failures.get(prob_idx, 0) + 1
                        detailed_failures.append((userid, prob_idx, msg))
                    
                    test_idx += 1
                
                # Track problem scores for summary
                if prob_idx not in summary_scores:
                    summary_scores[prob_idx] = []
                summary_scores[prob_idx].append(prob_score / pts)
    ...
    return user_grades, max_score, summary_scores, problem_failures, detailed_failures
...
def write_grading_summary(config: ConfigManager, summary_scores: dict, problem_failures: dict, 
                       detailed_failures: list, user_grades: dict, max_score: float) -> None:
    """Write grading summary with statistics."""
    summary_path = os.path.join(config.get_homework_dir(), "grading_summary.txt")
    
    with open(summary_path, "w", encoding="utf-8") as f:
        # Write problem statistics
        f.write("Problem Index,Avg Percentage,Failures\n")
        for prob_idx in sorted(summary_scores.keys()):
            avg_pct = (sum(summary_scores[prob_idx]) / len(summary_scores[prob_idx]) 
                      if summary_scores[prob_idx] else 0)
            failures = problem_failures.get(prob_idx, 0)
            f.write(f"{prob_idx},{avg_pct:.2%},{failures}\n")
        
        # Write detailed failure information
        if detailed_failures:
            f.write("\nDetailed Failures (User, Problem, Message):\n")
            for userid, prob_idx, msg in detailed_failures:
                f.write(f"{userid}, Problem {prob_idx}: {msg}\n")
        ...
        if valid_scores:
            ...
            f.write(f"({(avg_total_score/max_score):.2%})\n")
            f.write(f"Number of submissions: {len(valid_scores)}\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate manual feedback for student submissions")
    parser.add_argument("--footer", "-f", help="Custom footer message for feedback files")
    args = parser.parse_args()
    
    config = ConfigManager()
    
    print("Generating manual feedback from test results...")
    user_grades, max_score, summary_scores, problem_failures, detailed_failures = generate_manual_feedback(config, footer=args.footer)
    
    print("Writing grading summary...")
    write_grading_summary(config, summary_scores, problem_failures, detailed_failures, user_grades, max_score)
